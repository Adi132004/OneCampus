...
export function MarketplacePage() {
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-wrap gap-2">
          {CATEGORIES.map((c) => (
            <button
              key={c}
              onClick={() => setCat(c)}
              className={`rounded-full border px-4 py-1.5 text-xs font-medium transition-colors ${cat === c ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card text-foreground/80 hover:bg-muted"}`}
            >
              {c}
            </button>
          ))}
      ...
      <h2 className="mb-4 font-display text-xl font-semibold text-foreground">Featured</h2>
      ...
      <h2 className="mb-4 font-display text-xl font-semibold text-foreground">Recent listings</h2>
      {filtered.length === 0 ? (
        <EmptyState />
      ) : (
